...
Les principales fonctionnalités d'AI Data Engine (AIDE) fonctionnent de concert pour automatiser, sécuriser et accélérer le cycle de vie des données d'IA. Chaque fonctionnalité est implémentée sous forme d'un ensemble de microservices exécutés sur des DCN, intégrés au stockage ONTAP et exposés via des API REST et des interfaces de gestion.
