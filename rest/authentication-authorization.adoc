...
ingénieur de données:: Il s'agit d'un rôle administratif ONTAP prédéfini pour l'AIDE Data Engineer. Il restreint l'accès uniquement aux points de terminaison, ainsi qu'aux répertoires de commandes CLI correspondants, nécessaires pour effectuer les tâches d'ingénierie des données AIDE. Les tâches incluent la gestion des espaces de travail et des collections de données, la consultation des jobs et l'utilisation de l'aperçu de fichiers lorsque cela est autorisé.
